...
配布用として表紙に最低限のブランド情報を追加（本文は無変更）。

- 追加：ブランドバッジ3点 `無料配布PDF` / `2026年版` / `教員向け`（淡い青のピル表示・装飾は控えめ）。
- 変更：提供表記を「無料配布版」→「無料配布PDF（2026年版）」。
- 既存：ブランド名「教育DXナビ」・タイトル・サブタイトル・冒頭免責は維持。
- CSS は `.cover` 配下にスコープ追加したため、本文・印刷シートへの影響なし（再生成後の画像で確認済み）。

### 最終QA（表紙調整後・2026-06-29）

| # | 確認 | 結果 |
|---|---|---|
| 1 | 7ページ前後を維持 | ✅ 7ページ |
| 2 | 表紙が独立 | ✅ 1ページ目 |
| 3 | 表紙にブランド情報（教育DXナビ／無料配布PDF／2026年版／タイトル） | ✅ すべて表示 |
| 4 | 本文の □ が崩れない | ✅ 正常 |
| 5 | 印刷シートが見やすい | ✅ 正常（無変更） |
| 6 | 注意書きが残存 | ✅ 表紙免責・前提・項目②③・シート脚注 |
| 7 | 有料導線が強すぎない | ✅ 「準備中」基調のまま |
| 8 | 実在児童・勤務校・校内資料の想起 | ✅ なし |
